...
describe('useApiTags hook', () => {
  ...
  test('successful get tags query should report success', async () => {
    server.use(rest.get('*', (req, res, ctx) => res(ctx.status(200))));
    const { result, waitFor } = renderHook(() => useGetTags(), {
      wrapper,
    });
  ...
  test('error get tags query query should report error', async () => {
  ...
  test('timeout failure get tags query should report a timeout error', async () => {
